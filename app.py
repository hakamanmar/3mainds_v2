...
def init_db():
    ...
    # HIDDEN Internal group for Representative Chat functionality
    # This ID will NOT appear in any academic UI lists due to API filtering
    c.execute("INSERT OR IGNORE INTO sections (id, name) VALUES ('REPRESENTATIVES', '__INTERNAL_REPRESENTATIVES_GROUP__')")
...
def get_all_sections():
    try:
        conn = get_db()
        # EXCLUDE 'REPRESENTATIVES' from academic section lists
        rows = conn.execute("SELECT * FROM sections WHERE id != 'REPRESENTATIVES'").fetchall()
        conn.close()
...
def section_mgmt_init():
    conn = get_db()
    try:
        # Get academic sections (excluding Special groups like REPRESENTATIVES)
        sections_rows = conn.execute("SELECT id, name FROM sections WHERE id != 'REPRESENTATIVES'").fetchall()
...
def debug_users_sections():
    ...
    try:
        ...
        ''').fetchall()
        sections = conn.execute("SELECT id, name FROM sections WHERE id != 'REPRESENTATIVES'").fetchall()
        return jsonify({
...
def get_chat_messages():
    ...
    try:
        ...
        if receiver_id:
            ...
            is_rep_group = (section_id == 'REPRESENTATIVES' and ctx['role'] in ['section_admin', 'super_admin'])
...
def send_chat_message():
    ...
    section_id = ctx['section_id']
    if data.get('section_id') == 'REPRESENTATIVES':
        section_id = 'REPRESENTATIVES'
    elif ctx['role'] in ['super_admin', 'head_dept'] and data.get('section_id'):
        section_id = data.get('section_id')
    ...
    try:
        if not receiver_id:
            ...
            if ctx['role'] not in ['super_admin', 'head_dept']:
                # Virtual sections like REPRESENTATIVES are never locked via DB
        ...
        try:
            ...
            if receiver_id:
                ...
                if str(section_id).upper() == 'REPRESENTATIVES':
                    # Special case: section_admins + super_admins ONLY
                    others = conn.execute("""
                        SELECT id FROM users 
                        WHERE role IN ('section_admin', 'super_admin') 
                          AND id != ?
                    """, (ctx['user_id'],)).fetchall()
                else:
...
def get_chat_group_members(sid):
    ...
    try:
        ...
        if str(sid).upper() == 'REPRESENTATIVES':
            # STRICT FILTER: Representatives + Super Admins
...
def get_my_chat_groups():
    ...
    try:
        ...
        # 2. Add Virtual "Representatives Group" for Representatives + Super Admins
        if ctx['role'] in ['section_admin', 'super_admin']:
            # Find mute status for this virtual section
            mute_status = conn.execute('SELECT is_muted FROM chat_settings WHERE user_id = ? AND section_id = ?', 
                                     (ctx['user_id'], 'REPRESENTATIVES')).fetchone()
            section_res.append({
                'id': 'REPRESENTATIVES',
                'name': 'مجموعة ممثلي الشعب',
                'type': 'group',
                'is_locked': False,
                'is_muted': bool(mute_status['is_muted']) if mute_status else False
            })
