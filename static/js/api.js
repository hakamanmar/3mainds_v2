...
export const api = {
    ...
    async getSections() {
        ...
        // Robustly handle both {sections: []} and direct [] responses
        if (Array.isArray(res)) return res;
        if (res && Array.isArray(res.sections)) return res.sections;
        return [];
    },
    getSelectedSection() {
    ...
    async login(email, password) {
        ...
        // 🛑 Store CSRF token if returned (Critical for subsequent POST calls)
