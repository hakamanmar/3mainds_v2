...
SectionManagementPage.init = async () => {
    ...
    const showStudentProfile = async (studentId) => {
        UI.toast('جاري تحميل السجل الأكاديمي المحدث...', 'info');
        try {
            // Added timestamp to force fresh fetch and bypass any stuck browser cache
            const data = await api._fetch(`/api/admin/student-profile?student_id=${studentId}&_=${Date.now()}`);
